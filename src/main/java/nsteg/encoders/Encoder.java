 ...
 * Also carries out the encoding process, when Main or CLIParser request it, through the encode() method.
 * <p><br>
 * The encoding order is as follows:
 * [# of least significant bits used (4 bits)] - [Number of files encoded] - [Individual file sizes] -
...
public abstract class Encoder {
	 ...
	 * untouched. The file(s) can be optionally encrypted through the use of the AES cipher, using a 256 bit key derived
	 * by scrypt, which is generated by a user chosen password, and a random salt. The file(s) is/are always compressed
	 * before any encryption happens.
	 * <p><br>
	 * Instead of encoding each file individually, the files are loaded into a byte array, which contains all the files.
	 * Since the sizes of each file are encoded too, the files can be separated during the decoding phase.
	 *
	 ...
	 *
	 * @param filesToEncode Array of strings containing all the filenames of the files that will be encoded in the media
	 *                      file
	 ...
	 * @return Continuous byte array containing all the files to encode
	 */
