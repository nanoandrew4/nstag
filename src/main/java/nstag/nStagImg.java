...
public class nStagImg extends nStag {
	...
	public static void encode(String origPath, String fileToEncode, String outName, int bitsPerChannel) {
		byte[] metadataBits, dataBits, fileBytes;
		try {
...
//			Spinner.spin();
			...
			metadataBits = new byte[SIZE_BITS_COUNT + BITS_PER_CHANNEL_BITS + KEY_BITS_COUNT];
		} catch (IOException e) {
			System.err.println("Input files could not be found, please input the correct path, name and extension");
			return;
		}

		byte[][] keyAndDataBits = offerToEncrypt(fileBytes);
		byte[] keyBits = keyAndDataBits[0];
		dataBits = new byte[keyAndDataBits[1].length * 8];

		// Queue of bits to be encoded in the image
		int[] bitsArr;
		int pos = 0;
		for (byte fb : keyAndDataBits[1]) {
			bitsArr = intToBitArray(fb, 8, true);
			for (int db : bitsArr)
				dataBits[pos++] = ((byte) db);
		}
...
//			Spinner.spin();
	...
	public static void decode(String encodedPath, String outFileName) {
		...
		for (int j = 0; j < pxHeight; j++)
			for (int i = j == 0 ? 9 : 0; i < pxWidth && (i + j * pxWidth - 9) * 4 < bitsInImage + KEY_BITS_COUNT; i++)
				extractDataFromPixel(encoded.getRGB(i, j), bitsPerPixel, bits);

		byte[] keyBytes = new byte[KEY_BITS_COUNT / 8];
		...
		Spinner.spin();
		decodedBytes = offerToDecrypt(decodedBytes, keyBytes);
